...
const Sidebar = ({ onclick }) => {
  ...
  const { otherUsers, userProfile, buttonLoading, screenLoading } = useSelector(
  ...
  return (
    <div className="flex flex-col h-screen">
      <h2 className="text-2xl font-semibold text-start pb-2 mt-7 px-5">
        Chat Up!
      </h2>
      ...
      <div
        className={`h-full ${
          screenLoading ? "overflow-y-hidden" : "overflow-y-auto"
        } px-5`}
      >
        {screenLoading ? (
          <DataLoader />
      ...
      <div className=" flex items-center justify-between pt-2 mb-4 px-5">
        {screenLoading ? (
          ...
          <Link to={`/profileupdate`}>
            <div className="avatar flex items-center">
              ...
              <h6 className="mx-4 font-bold text-xl">
        ...
        <div>
          <button
            className="bg-blue-600 text-md font-semibold cursor-pointer hover:opacity-[0.9] active:translate-y-[1px] px-4 py-2 rounded-sm text-lg"
            onClick={handleLogout}
          >
            {buttonLoading ? (
              <div className="loader  h-7 w-7 mx-4"></div>
            ) : (
              <p>Logout</p>
            )}
          </button>
